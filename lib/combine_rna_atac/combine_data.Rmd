...
```{r}
get_close_peaks <- function(gene_id, chr, tss, window_size=100000) {
  ...
  overlaps[, overlap_distance := ifelse(tss <= peak_end & peak_start <= tss, 0,    
                                        ifelse(peak_end < tss, peak_end-tss, peak_start-tss))]  # may be possible to simplify
...
overlap_calc <- function(window_size=100000) {
  ...
  for (i in 1:nrow(rna_dt)) {
    ...
    val <- get_close_peaks(genes[i, gene_id], genes[i, gene_chr], genes[i, tss], window_size=window_size)
